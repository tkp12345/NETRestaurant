...
// 리액트+타입스크립트(React.FC<>) - https://react.vlpt.us/using-typescript/02-ts-react-basic.html
const MapContainer: React.FC<MapContainerProps> = ({ category,setDB,DB} ) => {
console.log('kako:',kakao);

  //const [rest,setRest]= useState([]);
  const [placeList,setPlaceList]=useState([]);

  const onclickLike = useCallback((place:any)=>{
      console.log('click 정보:',place)
      const data = JSON.parse(localStorage.getItem('DB')!)
      console.log('data:',data);
      const newData = data.forEach((v: any)=>{
          if(v.id === place.id){
              v.like=v.like+1;
          }
      })
...
function placesSearchCB(data: any, status: any, pagination: any) {
    //localstorage
    const newDB=[...data.filter((v: any)=>DB.every((s:any)=>s.id !== v.id))];
    const likeAdd =newDB.forEach((v)=>{v.like=0});
    // const newarr=DB.push(...data.filter(v=>DB.every(s=>s.id !== v.id)));
    const newarr=DB.push(...data.filter((v: any)=>DB.every((s:any)=>s.id !== v.id)));
    ...
    <ul id="category">
        ...
        <li id={CATEGORY.CONVEN} data-order="5">
          ...
          <RightListSection>
              ...
              { localStorage.getItem('DB') ?(JSON.parse(localStorage.getItem('DB')!).map((place:any) => {
                  return(
